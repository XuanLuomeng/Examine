...
public class ChangeGroup extends Application {
    // 接收修改的组名
    ...
    public void start(Stage St){
        ...
        confirm1.setOnAction(e->{
            //判断文本框不能为空且不能与已有组名相同
            if(!"".equals(GN.getText())&&(snackList.add(GN.getText()))) {
                HashSet Sn = new HashSet(snackList);
                snackList.clear();//先清空原本集合中的元素
                Iterator ite = Sn.iterator();
                while(ite.hasNext()){
                    String xv = (String)ite.next();
                    if(xv==groupName)
                        snackList.add(GN.getText());
                    else
                        snackList.add(xv);
                }
                ModifyGroupName mo0 = new ModifyGroupName();//将原本组别里的人员重新设定为新组名
                mo0.AllModifyGroupName(groupName,GN.getText());
